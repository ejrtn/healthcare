...
*최고 성능(v11, AUC 0.812+) 모델의 부상(Injury) 판별 검증 성능*
...
3D CT 스캔 데이터를 여러 개의 2D 슬라이스로 처리하고, 그중 부상이 의심되는 핵심 슬라이스에 집중(Attention)하여 최종적으로 부상 여부(any_injury)를 판별하는 구조입니다. V11은 여러 장기 분석보다 '전체 부상 유무' 판별에 집중하여 최고 성능(AUC 0.812+)을 달성한 모델입니다.
...
|:---|:---:|:---:|:---:|:---|
| **v1_2** | 128x128 | 13 | 0.7738 | Baseline. 2 Epoch 동결, `any_injury` 가중치 손실함수 적용. |
| **v2** | 128x128 | 9 | 0.6793 | 초반 10 Epoch 장기 동결 실험. 학습 속도 저하 확인. |
| **v3** | 128x128 | 13 | 0.6892 | Transformer Encoder 도입. 슬라이스 간 관계 학습 시도. |
| **v4** | 128x128 | 10 | 0.7070 | Gating 구조 도입. 부상 확률을 장기 헤드에 곱해 오탐 억제 시도. |
| **v5_2** | 224x224 | 16 | 0.7529 | 복부 CT 정밀 판독을 위해 입력 해상도 상향. 성능 비약적 상승. |
| **v6_4** | 224x224 | 21 | 0.7524 | 장기 별 가중치 및 Label Smoothing 적용. |
| **v7** | 224x224 | 11 | 0.7474 | Gradient Accumulation 도입 및 규제 강화. |
| **v8** | 224x224 | 2 | 0.6951 | Augmentation 완전 제거 시 성능 폭락 확인 (강건성 입증). |
| **v9** | 224x224 | 4 | 0.7131 | 증강 복구 및 LLRD (Layer-wise LR Decay) 전략 최초 도입. |
| **v10** | 224x224 | 8 | 0.7791 | 고해상도 Attention 루틴 최적화. |
| **v11** | 224x224 | 13 | 0.8124 | 부상 유무 판별에 집중하여 최고 성능 달성. |
| **v12** | 224x224 | 10 | 0.8047 | BCEWithLogitsLoss 전환 실험. 수렴 속도 개선 및 최적 임계값 확보. |
| **v13** | 224x224 | 8 | 0.7842 | Transformer 재도입 실험. 아키텍처 단순화 버전 대비 효율성 한계 확인. |
...
| **v10-v11** | 성능 임계치 돌파 | V11에서 부상 판별에만 집중했을 때 AUC 0.81 초과 달성. 의사 결정 우선순위의 중요성 확인. |
