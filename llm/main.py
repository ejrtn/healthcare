...
async def chat(request: ChatRequest):
    ...
    if not diseases:
        ...
        if assistant_turns < MAX_FOLLOWUP_ROUNDS and len(candidate_names) > AMBIGUITY_MAX_DISEASES:
            print(f"  후보 {len(candidate_names)}개(10개 초과) -> 증상 추가 요청")
            return {
                "answer": (
        ...
        if len(candidates) >= AMBIGUITY_MIN_DISEASES:
            ...
            # 후보 병명들을 나란히 보여준 뒤, "여러 질환이 예상된다" 식으로 답하게 한다.
            candidate_names = [c.get("disease") for c in candidates]
            ...
            multi_note = (
                f"증상만으로는 한 가지 질환으로 단정하기 어렵습니다. {', '.join(candidate_names)} 등 "
                "여러 질환의 가능성을 '~등 여러 질환이 예상됩니다'처럼 나열해서 안내하고, 그중 하나로 "
                "확정하지 마세요. 진료과도 특정 과 하나로 단정하지 말고, 증상이 지속되거나 심해지면 "
                "가까운 병원(내과 등)에서 진료를 받아보시길 권장하는 정도로만 안내하세요. "
